...
timer_details = [
    {'name': 'Timer Uno', 'duration': 172800, 'image': 'image1.png'},
    ...
    with col2:
        if st.session_state[timer['name']]:
            ...
            if now < st.session_state[timer['name']]:
                st.markdown(f"<h3 style='font-size: 20px;'>{timer['name']}: {format_timedelta(st.session_state[timer['name']] - now)}</h3>", unsafe_allow_html=True)
            else:
