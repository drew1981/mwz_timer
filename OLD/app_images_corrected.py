...
st.title('MWZ Tempo Di Ricarica')
...
for i in range(0, 8, 2):
    ...
    with col1:
        if st.image(os.path.join('images', image_files[i]), use_column_width=True) and st.button(f'Avvia Timer {i+1}'):
    ...
    with col2:
        if len(image_files) > i+1:  # Verifica che l'immagine esista prima di usarla
            if st.image(os.path.join('images', image_files[i+1]), use_column_width=True) and st.button(f'Avvia Timer {i+2}'):
